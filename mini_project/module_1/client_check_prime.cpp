...

{
 ...
 temp=recv(sockfd1,buf,5*sizeof(int),0);
	...
	if(buf[0]==2 || buf[0]==3 || buf[0]==5 || buf[0]==7)
...
{
	strcpy(rec_data,"given number is prime number");
...
{
	strcpy(rec_data,"given number is not prime number");

	cout<<"not prime";

}
	temp=send(sockfd1,rec_data,sizeof(rec_data),0);
