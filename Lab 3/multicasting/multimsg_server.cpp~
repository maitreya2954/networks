...
int main(int argc,char* argv[])
{
	...
	cout<<"Enter the number of clients for multicasting:";
	cin>>n;
	cout<< "Enter the clients for multicasting:";
	for (i=0;i<n;i++) 
	{
	  cin>>a[i];
	}
	...
	{
	 k=a[i];
	temp=send(client_fd[k],buf,strlen(buf)+1,0);
  	if(temp==-1)
    	{
  	...
  	char rec_data[50];
    	...
    	temp=recv(client_fd[k],rec_data,sizeof rec_data,0);
   		if(temp!=-1)
		{
